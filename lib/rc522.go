...
func (r *Reader) read(timeout time.Duration) ([3]string, error) {
	...
	b0, err := r.rfid.ReadCard(timeout, commands.PICC_AUTHENT1B, 2, 0, mfrc522.DefaultKey)
	if err != nil {
		switch {
	...
	b1, err := r.rfid.ReadCard(timeout, commands.PICC_AUTHENT1B, 2, 1, mfrc522.DefaultKey)
	if err != nil {
		return [3]string{}, err
	}

	b2, err := r.rfid.ReadCard(timeout, commands.PICC_AUTHENT1B, 2, 2, mfrc522.DefaultKey)
	if err != nil {
		return [3]string{}, err
	}
